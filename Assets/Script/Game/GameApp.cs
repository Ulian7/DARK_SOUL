...
public class GameApp : UnitySingleton<GameApp>
{
	...
	public void EnterGame()
	{
		...
		GameObject player = GameObject.Instantiate(playerPrefab, startPos, Quaternion.Euler(0, 0, 0));
		player.name = playerPrefab.name;

		GameObject ghostPrefab = ResMgr.Instance.GetAssetCache<GameObject>("Prefab/Ghost.prefab");
		GameObject ghost = GameObject.Instantiate(ghostPrefab, player.transform.position, Quaternion.Euler(0, 0, 0));
		...
		cinemachineFreeLook.LookAt = ghost.transform.Find("LookAt");
